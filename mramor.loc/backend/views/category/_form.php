...
<div class="category-form">
    ...
    <fieldset>
        ...
        <?= $form->field($model, 'file')->fileInput(['id'=>'fileupload', 'data-url'=>"index.php?r=category/download-img"]); ?>
    ...
    <fieldset>
        ...
        <?= $form->field($model, 'file_two')->fileInput(['id'=>'fileupload_2', 'data-url'=>"index.php?r=category/download-cover"]); ?>

        <?= $form->field($model, 'cover')->hiddenInput(['id' => 'picture_link_2']) ?>
    ...
    <fieldset>
        ...
        <?= $form->field($model, 'file_three')->fileInput(['id'=>'fileupload_3', 'data-url'=>"index.php?r=category/download-gallery", 'multiple'=> "multiple"]); ?>
        ...
        <div id="img_block_3">
            <?php
                ...
                foreach($model->getGallery() as $image){
                    echo $this->render('@app/views/category/_gallery_item', [ 'item' => ['image'=>$image]]);
                }
            ?>
        </div>
    ...
    $(function(){
        ...
        $('#fileupload_2').fileupload({
            dataType: 'json',
                ...
                var host = window.location.host.toString();
                var img = '<img src="http://'+host+data.result.link+'">';
                var block = $('#img_block_2');
                block.find('img').remove();
                block.append(img);
                $('#picture_link_2').val(data.result.link);
            }
        });
