...
<div class="stone-form">
    ...
    <?= $form->field($model, 'filter_one')->dropDownList(
        \yii\helpers\ArrayHelper::map(\backend\models\TypeOfStone::find()->all(),'id','name'),['prompt'=>'Тип камня']
    ) ?>

    <?= $form->field($model,'filter_two')->dropDownList(
        \yii\helpers\ArrayHelper::map(\backend\models\Colors::find()->all(),'id','name'),['prompt'=>'Цвет']
    ) ?>
    <?= $form->field($model, 'filter_three')->dropDownList(
        \yii\helpers\ArrayHelper::map(\backend\models\Country::find()->all(),'id','name'),['prompt'=>'Страна']
    ) ?>
    ...
    <?= $form->field($model, 'file')->fileInput(['id'=>'fileupload', 'data-url'=>"index.php?r=stone/test-img"]); ?>
    ...
    <fieldset>
        ...
        <?= $form->field($model, 'file_three')->fileInput(['id'=>'fileupload_3', 'data-url'=>"index.php?r=stone/download-gallery", 'multiple'=> "multiple"]); ?>
        ...
        <div id="img_block_3">
            ...
            foreach($model->getGallery() as $image){
                echo $this->render('@app/views/stone/_gallery_item', [ 'item' => ['image'=>$image]]);
            }
            ?>
        </div>
